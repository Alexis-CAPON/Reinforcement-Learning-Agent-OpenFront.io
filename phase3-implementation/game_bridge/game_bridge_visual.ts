...
class VisualGameBridge {
  ...
  async initialize(numBots: number, mapName: string = 'plains'): Promise<VisualState> {
  ...
  attackDirection(direction: string, intensity: number, build: boolean) {
    if (!this.game || !this.rlPlayer) {
      throw new Error('Game not initialized');
    }
