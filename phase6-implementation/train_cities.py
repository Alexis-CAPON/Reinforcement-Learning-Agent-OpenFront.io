"""
Training script for Phase 6 - Cities Only (Simplified)

Features:
- Economic system: Cities only (no ports, factories, etc.)
- Territory control with cluster awareness
- 2,500 action space (simplified from 38,500)
- Simplified observation space (8 spatial channels, 16 global features)
- Reward shaping for territory, cities, and gold

Expected training time: 15-25 hours on GPU (faster than Phase 5)
"""
...
def train_cities(
    ...
    """
    Train agent with cities-only (simplified Phase 6).
    ...
    logger.info("=" * 80)
    logger.info("PHASE 6 TRAINING - Cities Only (Simplified)")
    logger.info("=" * 80)
    logger.info(f"Map: {map_name}")
    logger.info(f"Opponents: {num_bots} bots")
    logger.info(f"Parallel envs: {n_envs}")
    logger.info(f"Total timesteps: {total_timesteps:,}")
    logger.info(f"Action space: 5 × 10 × 5 × 10 = 2,500 actions (simplified from 38,500)")
    logger.info(f"Observation space: 128×128×8 channels + 16 global features")
    logger.info(f"Features: Cities only, Territory control, Action masking")
    logger.info("=" * 80)

    # Create directories
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    model_dir = f'models/ppo_cities_{timestamp}'
    log_dir = f'logs/ppo_cities_{timestamp}'
    ...
    logger.info(f"Estimated time: 15-25 hours on GPU, 50-75 hours on CPU (faster than Phase 5)")
    logger.info(f"Monitor with: tensorboard --logdir {log_dir}")
    logger.info("=" * 80)

    try:
        # TEMPORARY: Disable eval_callback to test if it's causing 50k freeze
        model.learn(
            total_timesteps=total_timesteps,
            callback=[checkpoint_callback],  # Removed eval_callback temporarily
            progress_bar=True
        )
    ...
    logger.info(f"2. Test model: python test_cities.py --model {final_model_path}")
    logger.info(f"3. Visual test: python visual_test_cities.py --model {final_model_path}")
    logger.info("=" * 80)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train Phase 6 agent - Cities only (simplified)')
    ...
    if torch.cuda.is_available():
        ...
        logger.warning("Consider using a GPU for training (15-25 hours vs 50-75 hours)")
