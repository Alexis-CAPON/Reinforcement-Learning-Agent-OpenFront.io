...
class GameWrapper:
    ...
    def _start_bridge(self):
        ...
        # Start background thread to consume stderr immediately (prevents buffer deadlock)
        # CRITICAL: This must start BEFORE any significant stderr output accumulates
        ...
        # Note: stderr check won't work here since thread is consuming it,
        # but poll() will tell us if process died
        if self.process.poll() is not None:
            raise RuntimeError(f"Game bridge failed to start (process terminated immediately)")
    ...
    def _send_command(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not response.get('success', False):
                ...
                # Expected failures for game actions (not enough resources, invalid position, etc.)
                # These are normal game states, not errors - just return False
                if command_type in ['attack_direction', 'build_unit', 'launch_nuke']:
                    logger.debug(f"{command_type} action failed: {error}")
                    return response  # Return response with success=false, caller handles it

                # Actual errors for other command types (e.g., get_state, start_game)
