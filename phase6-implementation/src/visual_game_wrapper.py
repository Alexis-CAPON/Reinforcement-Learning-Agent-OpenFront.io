...
class VisualGameWrapper:
    ...
    def _read_stderr(self):
        ...
        try:
            for line in self.process.stderr:
                # Only print important bridge messages (skip verbose debug logs)
                line_stripped = line.rstrip()
                if any(keyword in line_stripped for keyword in ['Error', 'error', 'Warning', 'warning', 'Failed', 'failed', 'initialized', 'Initialized']):
                    print(f"[BRIDGE] {line_stripped}")
    ...
    def _send_command(self, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Note: Large responses (>1MB) work fine with readline() in text mode
        response_str = self.process.stdout.readline()

        if not response_str:
            if self.process.poll() is not None:
                raise RuntimeError("Bridge died unexpectedly")
        ...
        try:
            ...
            logger.error(f"Failed to parse JSON response (length={len(response_str)}): {response_str[:1000]}...")
            raise
