"""
OpenFront.io Phase 5 Environment - Large Map Support

Extends Phase 3 environment with:
- Support for larger maps (up to 500x500+)
- Improved downsampling for better spatial awareness
- Adaptive observation scaling based on map size
- Same observation space as Phase 3 (for compatibility)
"""
...
# Import base environment from Phase 3
...
class OpenFrontEnvLarge(gym.Env):
    """
    Phase 5 OpenFront.io Environment with large map support.

    Key improvements:
    - Supports maps from 100x100 to 1000x1000
    - Adaptive downsampling based on map size
    - Same 128×128×5 observation space (compatible with Phase 3 models)
    ...
    def __init__(
        ...
        if game_interface is None:
            try:
                ...
                logger.info(f"Created GameWrapper: map={map_name}, players={num_bots+1}")
        ...
        # Detect map size
        self.map_size = self._detect_map_size(map_name)
        logger.info(f"Map size detected: {self.map_size}x{self.map_size}")

        # Frame stacking for temporal context
        self.frame_buffer = deque(maxlen=frame_stack)

        # Observation space (same as Phase 3 for compatibility)
        ...
        self.last_action_was_wait = False
        self.recent_attack_directions = []
        self.last_direction = None

        logger.info(f"Large map environment initialized:")
        logger.info(f"  Map: {map_name} ({self.map_size}x{self.map_size})")
        logger.info(f"  Bots: {num_bots}")
        logger.info(f"  Frame stack: {frame_stack}")

    def _detect_map_size(self, map_name: str) -> int:
        """
        Detect map size from map name.

        Args:
            map_name: Name of the map

        Returns:
            Map size (width/height, assumed square)
        """
        ...
        if match:
            ...
            if width == height:
                ...
                logger.warning(f"Non-square map detected: {width}x{height}")
                return max(width, height)

        # Default sizes for common maps
        size_map = {
            'plains': 512,
            'world': 512,
            'europe': 512,
        ...
        # Default fallback
        logger.warning(f"Could not detect map size for '{map_name}', using 512")
        return 512
    ...
    def reset(
        ...
        logger.info(f"Episode {self.episode_count} started: {self.map_name}, {self.num_bots} bots")
    ...
    def step(
        ...
        if terminated:
            ...
            logger.info(
                ...
                f"  Map: {self.map_name} ({self.map_size}x{self.map_size})\n"
    ...
    def _extract_map(self, state) -> np.ndarray:
        ...
        Handles maps of any size by intelligently downsampling to 128×128.
        """
        if state is None:
    ...
    def _adaptive_downsample(self, array: np.ndarray) -> np.ndarray:
        ...
        Handles various input sizes intelligently:
        - 100×100 → upsample to 128×128
        - 128×128 → return as-is
        - 500×500 → downsample using average pooling
        - 1000×1000 → downsample using average pooling

        Args:
            array: Input array of any size
        ...
        if scale <= 2:
            # Simple 2×2 average pooling
            return self._avg_pool_2x2(array)
        elif scale <= 4:
            # 4×4 average pooling
            return self._avg_pool_4x4(array)
        elif scale <= 8:
            # 8×8 average pooling
            return self._avg_pool_8x8(array)
        else:
            # Very large map - use simple strided slicing
            stride = int(scale)
            return array[::stride, ::stride][:128, :128]

    def _avg_pool_2x2(self, array: np.ndarray) -> np.ndarray:
        """2×2 average pooling"""
        h, w = array.shape
        h_out, w_out = h // 2, w // 2
        return array.reshape(h_out, 2, w_out, 2).mean(axis=(1, 3))[:128, :128]

    def _avg_pool_4x4(self, array: np.ndarray) -> np.ndarray:
        """4×4 average pooling"""
        h, w = array.shape
        h_out, w_out = h // 4, w // 4
        return array.reshape(h_out, 4, w_out, 4).mean(axis=(1, 3))[:128, :128]

    def _avg_pool_8x8(self, array: np.ndarray) -> np.ndarray:
        """8×8 average pooling"""
        h, w = array.shape
        h_out, w_out = h // 8, w // 8
    ...
    def _extract_global(self, state) -> np.ndarray:
        """Extract 16 global features (same as Phase 3)."""
        if state is None:
    ...
    def _find_target_in_direction(self, direction: int):
        ...
        # Would need game interface implementation
        return None

    def _compute_reward(self) -> float:
        """Compute reward (same as Phase 3)."""
        if self.game is None:
            return 0.1
    ...
    def close(self):
        ...
        logger.info("Large map environment closed")
